package vehicleinsuranceproject;
...
public class Vehicleselenium {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		  ...
		  WebElement make = driver.findElement(By.xpath("//select[@id='make']"));
			...
			for(int i=0;i<companies.size();i++) {
				if(i==0) {
					actions.moveToElement(make).click().perform();
			...
			Actions actionsallcountries = new Actions(driver);
			actions.moveToElement(countries).click().perform();
			...
			actions.moveToElement(occupations).click().perform();
			...
			Actions allinsurances = new Actions(driver);
			allinsurances.moveToElement(insurancesum).click().perform();
			...
			Actions alldamages = new Actions(driver);
			alldamages.moveToElement(Damagesum).click().perform();
		   ...
		   Actions allcourtesyoptions = new Actions(driver);
		   allcourtesyoptions.moveToElement(Courtesycars).click().perform();
     ...
     WebElement dropdown = waits.until(ExpectedConditions.visibilityOfElementLocated(By.id("meritrating")));
     Select select = new Select(dropdown);
     select.selectByVisibleText("Bonus 3");

 
   WebElement Optionalproducts =driver.findElement(By.xpath("//input[@id='EuroProtection']"));
   Select soptionalproducts = new Select(Optionalproducts);
   soptionalproducts.selectByIndex(0);
